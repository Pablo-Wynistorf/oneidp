...
function displayAuthorizedApps(apps) {
  ...
  if (apps.length === 0) {
    container.innerHTML = '<div class="text-gray-400 text-center py-4">No authorized applications yet</div>';
    return;
  }
    ...
    <div class="bg-gray-700 rounded-lg p-4 flex items-center justify-between">
      <div class="flex-1">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-white font-semibold">${escapeHtml(app.appName)}</h3>
            <p class="text-gray-400 text-xs">${escapeHtml(app.clientId)}</p>
          </div>
        </div>
