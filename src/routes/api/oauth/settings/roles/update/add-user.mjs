...
import { userDB, oAuthRolesDB, oAuthClientAppDB, userAppConsentDB } from '../../../../../../database/mongodb.mjs';
...
router.post('/', async (req, res) => {
  ...
  try {
    if (basicAuth) {
      ...
      // The cookie path below is covered by requireAppManagement on the whole
      // /api/oauth/settings prefix; client-credential callers are not, so the
      // same rule is applied here.
      if (await rejectIfClientDisabled(client, res)) return undefined;
