...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    jwt.verify(access_token, JWT_PUBLIC_KEY, { algorithms: ['RS256'] }, async (error, decoded) => {
      ...
      // An application whose owner has lost app management cannot be used to
      // sign anyone in. Checked before consent so a disabled client cannot even
      // ask the user for authorisation.
