...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (grant_type === 'refresh_token') {
      ...
      // A disabled application cannot mint new tokens from a refresh token it
      // was handed while it was still live.
      if (await rejectIfClientDisabled(oauth_client, res, { status: 400 })) return undefined;
      ...
      // Rejected before the code is consumed or PKCE is verified: there is
      // nothing to exchange for an application that is switched off.
      if (await rejectIfClientDisabled(oauth_client, res, { status: 400 })) return undefined;
