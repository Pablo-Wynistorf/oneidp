...
import { notifyLogin, notifyError } from '../../../../notify/notifications.mjs';
...
router.post('/', async (req, res) => {
  ...
  if (!mfa_token) {
    ...
    // Re-checked here as well as at password entry: an account can be banned in
    // the window between the two steps.
    if (rejectIfBanned(userData, res)) {
      res.clearCookie('mfa_token');
      return undefined;
    }
